...
// === Mobile-safe Auth wiring (override) ===
(function(){
  const $ = (sel)=>document.querySelector(sel);
  const $$ = (sel)=>Array.from(document.querySelectorAll(sel));
  function xErr(e){
    const code = e && (e.code || e.message || e.toString());
    if(!code) return 'שגיאה';
    if(String(code).includes('auth/invalid-credential')) return 'אימייל או סיסמה שגויים';
    if(String(code).includes('auth/too-many-requests')) return 'נחסמת זמנית. נסה שוב מאוחר יותר';
    return code;
  }
  async function doSignIn(){
    ...
    try{
      ...
      $('#authModal')?.close?.();
    }catch(e){ if(errEl) errEl.textContent = xErr(e); console.error(e); }
  }
  async function doSignUp(){
    const email = $('#suEmail')?.value?.trim();
    const pass  = $('#suPass')?.value || '';
    const errEl = $('#suError');
    if(!email || !pass){ if(errEl) errEl.textContent='מלא אימייל וסיסמה (6 תווים לפחות)'; return; }
    try{
  ...
  async function doReset(){
    ...
    const info = $('#rsInfo');
    if(!email){ if(info) info.textContent='כתוב אימייל לאיפוס'; return; }
    try{
      await FB.sendPasswordResetEmail(auth, email);
      if(info) info.textContent='נשלח מייל לאיפוס';
    }catch(e){ if(info) info.textContent = xErr(e); console.error(e); }
  }
  function activeTab(){
    const btns = $$('.tabs .tab-btn');
    const act = btns.find(b=>b.classList.contains('active'));
    return act ? act.getAttribute('data-tab') : 'loginTab';
  }
  function swapTab(tab){
    $$('.tab-panel').forEach(p=>p.hidden = p.id!==tab);
    $$('.tabs .tab-btn').forEach(b=> b.classList.toggle('active', b.getAttribute('data-tab')===tab));
    const primary = $('#authPrimary');
    if(!primary) return;
    const t = tab;
    primary.textContent = (t==='loginTab') ? 'כניסה' : (t==='signupTab' ? 'הרשמה' : 'שלח קישור איפוס');
  }

  document.addEventListener('DOMContentLoaded', ()=>{
    // open auth when clicking header login
    const headerLogin = $('#btnLogin');
    if(headerLogin && !headerLogin.dataset._wired){
      headerLogin.dataset._wired='1';
    ...
    $$('.tabs .tab-btn').forEach(btn=>{
      btn.addEventListener('click', ()=>{
        swapTab(btn.getAttribute('data-tab'));
      }, {passive:true});
    });
    // Primary action dispatch
    const primary = $('#authPrimary');
    if(primary && !primary.dataset._wired){
      primary.dataset._wired='1';
      primary.addEventListener('click', async (e)=>{
        e.preventDefault();
        const t = activeTab();
        if(t==='loginTab') return doSignIn();
        if(t==='signupTab') return doSignUp();
        if(t==='resetTab') return doReset();
      }, {passive:false});
    }
    // Secondary closes
    const secondary = $('#authSecondary');
    if(secondary && !secondary.dataset._wired){
      secondary.dataset._wired='1';
      secondary.addEventListener('click', ()=> $('#authModal')?.close?.(), {passive:true});
    }
  });

  // Solid auth-state UI toggle (works on iOS Safari/Chrome)
  if (FB && FB.onAuthStateChanged){
    FB.onAuthStateChanged(auth, (user)=>{
      try { window.__authPrimarySwap && window.__authPrimarySwap(!!user); } catch(_){}
      const loginScreen = document.getElementById('loginScreen');
      const container = document.querySelector('.container');
      if(user){
        if(loginScreen) loginScreen.style.display = 'none';
        ...
        // Show centered login, hide app UI
  ...
  // Mobile logout buttons (header + inline)
  function bindMobileLogout(){
    const logoutBtns = $$('.btn-logout-mobile, #btnLogout, #btnLogoutMobile');
    logoutBtns.forEach(btn=>{
      if(btn.dataset._wired) return;
      btn.dataset._wired='1';
      btn.addEventListener('click', async (e)=>{
        e.preventDefault();
        try { await (window.hardSignOut ? window.hardSignOut() : FB.signOut(auth)); }
        catch(e){ console.warn('signout',e); }
      }, {passive:false});
    });
  }
  document.addEventListener('DOMContentLoaded', bindMobileLogout);
})();
