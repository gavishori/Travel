...
.container{display:grid;grid-template-columns:320px 1fr;height:calc(100dvh - 56px)}
...
@media (max-width: 480px) {
  ...
  #view-journal .expenses-table tr.exp-item {
    display: grid;
    gap: .25rem .5rem; /* Adjust gap */
    align-items: center;
    padding: .5rem .75rem; /* Add padding around the entire item */
    border-bottom: 1px solid var(--border); /* Add separator between items */
  }

  /* --- Expenses: two-line header layout --- */
  #view-expenses .expenses-table.stacked tr.exp-item {
    grid-template-columns: 1fr 1fr auto;
    grid-template-areas: "date time menu"
                         "amount currency menu";
  }

  /* Original rule that reset background is removed or commented out */
  #view-expenses .expenses-table.stacked td.cell {
    border: none;
    padding: 0;
    /* background: transparent; --- This line is removed/modified --- */
  }

  /* --- Apply header background to relevant cells ONLY in mobile Expenses view --- */
  #view-expenses .expenses-table.stacked td.header,
  #view-expenses .expenses-table.stacked td.menu-cell {
    background: color-mix(in oklab, var(--ink) 6%, transparent); /* Apply desired background */
    padding: 0.2rem 0.4rem; /* Add some internal padding */
  ...
  /* Style labels within the header cells */
   #view-expenses .expenses-table.stacked td.header .lbl {
      display: inline;
      font-weight: normal;
      opacity: 0.7;
      font-size: 0.8em;
  ...
  #view-expenses .expenses-table.stacked td.header.amount {
       grid-area: amount;
       flex-direction: column; /* Stack amount and rate vertically */
  ...
  #view-expenses .expenses-table.stacked td.menu-cell {
      grid-area: menu;
      text-align: left;
      justify-content: flex-end; /* Push menu button to the end */
  }
  #view-expenses .expenses-table.stacked td.header.category { display: none; } /* Hide category on mobile */

  /* Ensure notes cell stays transparent and spans full width */
  #view-expenses .expenses-table.stacked td.notes {
    grid-column: 1 / -1;
    padding: .5rem 0 .5rem 0;
    background: transparent !important; /* Override just in case */
    border-radius: 0; /* Remove rounding if applied above */
  }

  /* Mobile typography adjustments for header cells */
  #view-expenses .expenses-table.stacked .amount .amt-main { font-weight: 600; font-size: 0.9em; }
  #view-expenses .expenses-table.stacked .currency .ils-total { font-weight: 600; font-size: 0.9em; }
  #view-expenses .expenses-table.stacked .amt-sub { font-size: .75em; opacity: .8; }
  #view-expenses .expenses-table.stacked td.header.date,
  #view-expenses .expenses-table.stacked td.header.time { font-size: 0.9em; }


  /* --- Journal: (Existing rules remain mostly the same, ensure notes background is transparent) --- */
  #view-journal .expenses-table tr.exp-item {
    grid-template-columns: 1fr 1fr auto;
    grid-template-areas: "date time menu"
                         "location location menu";
  }
  #view-journal .expenses-table td.cell { border: none; padding: 0; background: transparent; } /* Keep transparent for Journal headers */
  ...
  #view-journal .expenses-table td.menu-cell { grid-area: menu; text-align: left; }
  #view-journal .expenses-table td.notes { /* Ensure notes cell background is transparent */
      grid-column: 1 / -1;
      padding:.25rem 0 .75rem 0;
      background: transparent !important;
  }

  /* Hide selection checkboxes by default on mobile */
  #view-journal .expenses-table td.select-cell,
  #view-expenses .expenses-table td.select-cell { display: none; }

  /* --- Trip meta (Existing rules remain) --- */
...
#jrText a.link-chip{ color:#1E88E5; font-weight:700; text-decoration:none; }
