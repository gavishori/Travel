// SPA Navigation & behavior (no external deps)
const screens = {
...
document.getElementById('btn-open-location-expense')
  .addEventListener('click', () => { locationContext = 'expense'; showScreen('set-location'); });

document.getElementById('btn-open-location-journal')
  .addEventListener('click', () => { locationContext = 'journal'; showScreen('set-location'); });

document.getElementById('btn-cancel-location')
...
document.getElementById('location-form')
  .addEventListener('submit', (e) => {
    ...
    let label = 'ללא מיקום'; let value = '';
    if (place || (lat && lng)) {
      const parts = []; if (place) parts.push(place); if (lat && lng) parts.push(`(${lat}, ${lng})`);
      label = parts.join(' ');
