...
async function doHardSignOut(){
  ...
  const ts=Date.now(); location.replace(FB.APP_BASE + (FB.APP_BASE.includes('?')?'&':'?') + 'logout=' + ts);
}
...
async function sendLink(){
  ...
  msg('שולח לינק...');
  try{ await FB.sendMagicLink(email, FB.APP_BASE); msg('נשלח לינק למייל. פתח/י אותו במכשיר הזה.'); }
  catch(err){ alert('שגיאה: ' + (err?.code||err?.message||err)); msg(''); }
}
