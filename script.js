...
// === Prevent page scroll when any modal dialog is open ===
(function(){
  ...
  document.addEventListener('DOMContentLoaded', ()=>{
    document.addEventListener('click', toggleBodyScroll, true);
    document.addEventListener('keydown', toggleBodyScroll, true);
  });
  const mo = new MutationObserver(toggleBodyScroll);
...
document.addEventListener("DOMContentLoaded", function(){
  ...
  if (editBtn)  editBtn.onclick = async ()=>{
    const trip = await Store.getTrip(state.currentTripId);
    if (!trip) return;
    const exp = (await Store.listExpenses(trip.id)).find(x=>x.id===__rowActionExp.expId);
    if (exp) openExpenseDialog(exp);
    closeExpenseRowActionsDialog();
  };
  if (delBtn)   delBtn.onclick  = ()=>{
    if (__rowActionExp.expId) removeExpense({ id: __rowActionExp.expId, tripId: state.currentTripId });
    closeExpenseRowActionsDialog();
  };
  dlg.addEventListener("cancel", closeExpenseRowActionsDialog);
});
document.addEventListener("click", (ev)=>{
  const btn = ev.target.closest && ev.target.closest(".kebab");
  if (!btn) return;
