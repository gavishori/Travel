...
function updateMap(filter = 'all') {
    ...
    initMapEnhanced(filteredPoints);
...
function initMap(points) {
    ...
    if (!map) {
        ...
        markerClusterGroup = L.markerClusterGroup({ disableClusteringAtZoom: 17, spiderfyOnEveryZoom: true, maxClusterRadius: 40, chunkedLoading: true });
    ...
    validPoints.forEach(p => {
        ...
        `;
        const marker = L.marker([p.lat, p.lng], { icon: isExpense ? expenseIcon : logIcon, data: Object.assign({type: (isExpense?'expense':'log')}, p) }).bindPopup(popupContent, { className: 'custom-popup' });
        markers.push(marker);
    });
...
// Fallback wrapper: enhanced map initializer (alias to initMap if custom impl missing)
function initMapEnhanced(points){
    if (typeof initMap === 'function') {
        return initMap(points);
    } else {
        // Minimal no-op guard to avoid crashes
        console.warn('initMap not defined; skipping map render');
        return;
    }
}


// === Enhanced map popup with actions + focus helpers ===
(function(){
  // Rebuild popup HTML for a point
  function buildPopupHTML(p){
    const isExpense = !!p.category;
    const title = isExpense ? 'הוצאה' : 'תיעוד יומי';
    const desc = (p.text || p.description || p.locationName || '').toString();
    const dateStr = p.date ? (function(d){ try{ const dt = new Date(d); const dd = dt.toLocaleDateString('he-IL'); const tt = dt.toLocaleTimeString('he-IL', {hour:'2-digit', minute:'2-digit'}); return `${tt} | ${dd}`;}catch(e){ return ''; } })(p.date) : '';
    const amount = isExpense && p.amount != null ? `<div class="text-sm font-extrabold mt-1">${p.currency || ''} ${p.amount}</div>` : '';
    const coords = (p.lat && p.lng) ? `<div class="text-xs opacity-70 mt-2">קואורדינטות: ${Number(p.lat).toFixed(4)}, ${Number(p.lng).toFixed(4)}</div>` : '';
    const id = p.id || p._id || p.uid;
    return `<div class="p-2 text-right">
      <div class="font-bold mb-1">${title}</div>
      ${amount}
      <div class="text-sm mt-1" style="white-space:pre-wrap;">${desc || ''}</div>
      <div class="text-xs opacity-70 mt-2">${dateStr}</div>
      ${coords}
      ${id ? `<div class="flex gap-2 mt-3 justify-start">
        <button class="popup-go px-2 py-1 rounded-md bg-blue-600 text-white text-xs" data-id="${id}" data-type="${isExpense ? 'expense':'log'}">פתח ברשימה</button>
        <button class="popup-edit px-2 py-1 rounded-md bg-emerald-600 text-white text-xs" data-id="${id}" data-type="${isExpense ? 'expense':'log'}">ערוך</button>
      </div>` : ''}
    </div>`;
  }

  // Focus helpers
  window.__tlFlashFocus = function(el){
    try{
      el.classList.add('ring-2','ring-blue-500','ring-offset-2','ring-offset-transparent');
      setTimeout(()=>{ el.classList.remove('ring-2','ring-blue-500','ring-offset-2','ring-offset-transparent'); }, 1600);
    }catch(e){}
  };

  window.focusExpense = function(id, doEdit){
    const root = document.querySelector(`[data-expense-id='${id}']`);
    if(root){
      root.scrollIntoView({behavior:'smooth', block:'center'});
      window.__tlFlashFocus(root);
      if(doEdit){
        const btn = root.querySelector('.edit-expense');
        if(btn) btn.click();
      }
    }
  };

  window.focusLog = function(id, doEdit){
    const root = document.querySelector(`[data-log-id='${id}']`);
    if(root){
      root.scrollIntoView({behavior:'smooth', block:'center'});
      window.__tlFlashFocus(root);
      if(doEdit){
        const btn = root.querySelector('.edit-log-btn');
        if(btn) btn.click();
      }
    }
  };

  // Hook into Leaflet popups if map exists later
  const hook = () => {
    if (typeof map === 'undefined' || !map) return;
    map.off('popupopen.__tl'); // idempotent
    map.on('popupopen.__tl', function(e){
      try{
        const src = e.popup && e.popup._source;
        const data = src && (src.options && src.options.data || src.feature && src.feature.properties);
        if(!data) return;
        e.popup.setContent(buildPopupHTML(data));
        const el = e.popup.getElement();
        if(!el) return;
        const go = el.querySelector('.popup-go');
        const edit = el.querySelector('.popup-edit');
        if(go){
          go.addEventListener('click', () => {
            const type = go.getAttribute('data-type'); const id = go.getAttribute('data-id');
            if(type === 'expense') window.focusExpense(id, false); else window.focusLog(id, false);
          }, {once:true});
        }
        if(edit){
          edit.addEventListener('click', () => {
            const type = edit.getAttribute('data-type'); const id = edit.getAttribute('data-id');
            if(type === 'expense') window.focusExpense(id, true); else window.focusLog(id, true);
          }, {once:true});
        }
      }catch(err){ /* noop */ }
    });
  };
  // try now and after small delay
  setTimeout(hook, 100);
  document.addEventListener('DOMContentLoaded', hook);
})();


// === Robust theme handler (light/dark) ===
(function(){
  const STORAGE = 'tl-theme';
  ...
  const apply = (mode) => {
    ...
    root.classList.toggle('dark', isDark); // for Tailwind dark
    try{ if (window.lucide) lucide.createIcons(); }catch(e){}
    try{ if (typeof map !== 'undefined' && map) setTimeout(()=> map.invalidateSize(), 60); }catch(e){}
    localStorage.setItem(STORAGE, mode);
  };
  let current = localStorage.getItem(STORAGE) || (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light');
  apply(current);
  if(btn){
