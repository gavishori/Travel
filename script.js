...
if (token && tripId) {
  ...
  // Only journal + map
  $$('#tabs button[data-tab]').forEach(b=>{ if(!['journal','map'].includes(b.dataset.tab)) b.style.display='none'; });
  showView('journal');
...
function renderExpenses(t, order){
  ...
  // One-time migration per trip: fix titles that accidentally became the numeric amount (e.g., "0.00000")
  try{ migrateBadExpenseTitles(t); }catch(_){}
...
arr.forEach(e=>{
    ...
    const _titleText = deriveTitleCandidate(e, t);
    const _href = buildMapHref(e);
    const title = _titleText ? (_href ? `<a class="title-link" href="${_href}" target="_blank" rel="noopener">${esc(_titleText)}</a>` : esc(_titleText)) : '';
    // If coords exist but we still have no title/location -> enrich in background
    const _needsGeo = (!(_titleText||'').trim()) && (e.lat!=null && e.lng!=null);
    ...
    tr1.dataset.id = e.id;
	    ...
	    // Compact: show only the paid amount in its currency. Details (rate + ₪) are in a small popover.
    tr1.innerHTML = `
      ...
      <td class="cell header title"><span class="lbl">כותרת:</span> ${title}</td>
	      <td class="cell header amount">
	        <div class="amt-main" style="display:flex; align-items:center; justify-content:flex-end; gap:6px;">
	          <button class="fx-btn" type="button" aria-label="פרטי המרה" data-fx="1" data-curr="${esc(curr)}" data-amt="${Number(e.amount||0)}" data-rate="${rateToILS ?? ''}" data-ils="${convertedAmountILS ?? ''}">▾</button>
	          <span class="code">${curr}</span> <span class="val">${amount}</span>
	        </div>
	      </td>
	      <td class="cell header currency" dir="rtl"></td>
    ...
    tr4.innerHTML = `<td class="cell notes" colspan="7">${desc||''}</td>`;
	    ...
	    // Remove all children from the amount cell except amt-main (prevent duplicates)
    ...
    if(_needsGeo){
      (async()=>{
        try{
          ...
          const cell = tr1.querySelector('.cell.header.title');
          if(cell){
            const href = buildMapHref({lat:e.lat,lng:e.lng,locationName:name});
            cell.innerHTML = `<span class="lbl">כותרת:</span> ` + (href ? (`<a class="title-link" href="${href}" target="_blank" rel="noopener">${esc(head)}</a>`) : esc(head));
          }
...
function renderJournal(t, order){
  try{
    ...
    const _destCity = guessDestinationCity(t);
    arr.forEach(j=>{
      ...
      // כותרת הדיווח (fallback: placeName -> destination city)
      const _rawTitle = (j.title||'').toString().trim();
      const _rawPlace = (j.placeName||'').toString().trim();
      const _nameFromPlace = _rawPlace ? _rawPlace.split(',')[0].trim() : '';
      const locStr = esc(_rawTitle || _nameFromPlace || _destCity || '');
...
function renderAllTimeline(t, order){
  try{
    ...
    const mode = (state && state.overviewMode) ? String(state.overviewMode) : 'all';
    let combined = expenses.concat(journal)
      .sort((a,b)=> dir * (expenseSortKey(a) - expenseSortKey(b)));
    if (mode === 'expenses') combined = combined.filter(x=>x.__kind==='expense');
    if (mode === 'journal') combined = combined.filter(x=>x.__kind==='journal');
    ...
    try{ if (typeof window.__overviewApplyAfterRender === 'function') window.__overviewApplyAfterRender(); }catch(_){ }
}

function appendExpenseRowToTimeline(body, e){
  try{
    const _tripObj = (state.current || state._lastTripObj || {});
    const d = dayjs(e.dateIso || e.createdAt);
    let dateStr = '', timeStr = '';
    if (d.isValid()){
    ...
    const cat    = esc(e.category || '');
    const _titleText = deriveTitleCandidate(e, _tripObj);
    const _href = buildMapHref(e);
    const title  = _titleText ? (_href ? `<a class="title-link" href="${_href}" target="_blank" rel="noopener">${esc(_titleText)}</a>` : esc(_titleText)) : '';
    ...
    tr1.dataset.kind = 'expense';
      ...
      <td class="cell header amount">
	        <div class="amt-main" style="display:flex; align-items:center; justify-content:flex-end; gap:6px;">
	          <button class="fx-btn" type="button" aria-label="פרטי המרה" data-fx="1" data-curr="${esc(curr)}" data-amt="${amountNum}" data-rate="${rateToILS ?? ''}" data-ils="${convertedAmountILS ?? ''}">▾</button>
	          <span class="code">${curr}</span> <span class="val">${amount}</span>
	        </div>
      </td>
    ...
    // Details row (notes). "צמצם הכל" should hide only these rows and keep the green/red header rows visible.
    tr2.className = 'exp-item exp-details';
    tr2.innerHTML = `<td class="cell notes" colspan="7">${desc || ''}</td>`;
...
function appendJournalRowToTimeline(body, j){
  try{
    ...
    const tr2 = document.createElement('tr');
    // Details row (notes). "צמצם הכל" should hide only these rows and keep the header row visible.
    tr2.className = 'exp-item exp-details';
